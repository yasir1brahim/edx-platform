...
def get_visible_courses(org=None, platform=None, filter_=None):
    """
    ...
    if org:
        ...
        if not current_site_orgs or org in current_site_orgs:
            courses = CourseOverview.get_all_courses(orgs=[org], platform=platform, filter_=filter_)
    elif current_site_orgs:
        # Only display courses that should be displayed on this site
        courses = CourseOverview.get_all_courses(orgs=current_site_orgs, platform=platform, filter_=filter_)
    else:
        courses = CourseOverview.get_all_courses(platform=platform,filter_=filter_)
